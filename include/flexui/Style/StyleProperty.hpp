...
namespace flexui {
	...
	bool ParseStylePropertyLine(const StringSection& line, std::vector<StyleProperty>& properties, ParseResult* pr = nullptr);

	/// Parse a full block of style properties separated by `;`
	/// The properties will be inserted into the `properties` vector
	/// (the vector will not be cleared)
	/// Returns true if at least one property was parsed
	bool ParseStylePropertiesBlock(const StringSection& block, std::vector<StyleProperty>& properties, ParseResult* pr = nullptr);
